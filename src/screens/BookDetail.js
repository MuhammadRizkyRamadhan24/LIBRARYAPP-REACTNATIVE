...
export class BookDetail extends Component {
    ...
    render() {
        console.log(this.state.book[0].bookImage)
        return (
            <View style={{flex: 1,backgroundColor: '#131212'}}>
                ...
                <View style={{flex: 8}}>
                    <View style={{flex: 7,justifyContent:'center'}}>
                        <Image source={{uri: `${this.state.book[0].bookImage}`}} style={{height:300,width:null,resizeMode:'contain',borderRadius:15}}></Image>
                    </View>
                    <View style={{flex: 2,justifyContent:'center',alignItems:'center',flexDirection:'column'}}>
                        <Text style={{fontFamily:'Gotham_Medium',fontSize:25,color:'white'}}>{this.state.book[0].title}</Text>
                        <Text style={{fontFamily:'SanFranciscoPro',fontSize:16,marginTop:20,color: '#838388'}}>{this.state.book[0].author}</Text>
                    </View>
                    <View style={{flex: 1,alignItems:'center',justifyContent:'center'}}>
                        {this.state.book[0].status == 'Available' || this.state.book[0].status == 'available'
                        ?
